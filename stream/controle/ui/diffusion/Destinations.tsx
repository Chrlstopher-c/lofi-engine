...
import type { Destinations as EtatDestinations, Diffusion } from "../../types.ts";
...
interface Props {
  conf: Diffusion;
  cles: Cles;
  /** L'état de chaque plateforme dans le flux en cours. null quand rien ne diffuse. */
  flux: EtatDestinations | null;
...
export function Destinations({ conf, cles, flux, definir, setCles }: Props): ReactNode {
  return (
    <Section titre="Destinations" compte={compte(conf)}>
      <Plateforme nom="Twitch" marque="twitch" actif={conf.twitchActif} cleEnregistree={conf.twitchCle}
        ingest={conf.twitchIngest} nouvelleCle={cles.twitch} etatFlux={flux?.twitch ?? null}
      ...
      <Plateforme nom="YouTube" marque="youtube" actif={conf.youtubeActif} cleEnregistree={conf.youtubeCle}
        ingest={conf.youtubeIngest} nouvelleCle={cles.youtube} etatFlux={flux?.youtube ?? null}
